...
- The MacBook Pro-side GUI controller now builds as `iBridge Studio.app`. The app has separate Sender, Receiver, and Logs tabs, a menu bar extra that survives closing the main window, dropdown presets for signal/bitrate/duration, generated app icons, and a `Restore Windows` action for pulling windows stranded on virtual displays back to the MacBook display area. It still requires macOS Accessibility permission for the restore action and source-side input injection.
