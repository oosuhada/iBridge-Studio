...
- A first MacBook Pro-side GUI controller now exists at `apps/controller-macos`. It builds `iBridge Control.app` into the alpha package and provides presets for 2015 5K quality, 2015 smooth, and 2017 4K quality, with buttons for display listing, receiver startup, sender startup, sender stop, and inline logs. This should replace hand-typed terminal command strings for normal lab use.
